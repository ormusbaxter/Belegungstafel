...
/* Meldestatus der Station: genau eine der drei Farben, keine leere Angabe.
   Eine Tafel ohne Meldestatus gibt es im Dienst nicht – wer nichts meldet,
   meldet grün. Ältere Stände ohne Angabe werden beim Einlesen darauf
   gebracht. */
const MELDE_WERTE = ['grün', 'gelb', 'rot'];
const MELDE_VORGABE = MELDE_WERTE[0];
const meldeWert = wert => (MELDE_WERTE.includes(String(wert)) ? String(wert) : MELDE_VORGABE);
